...
function Field({ label, name, required = false, type = "text", defaultValue = "", placeholder = "", disabled = false }: { label: string; name: string; required?: boolean; type?: string; defaultValue?: string; placeholder?: string; disabled?: boolean }) {
  return <label className="grid gap-2 text-sm font-black text-[#081221]"><span>{label}{required ? " *" : ""}</span><input name={name} type={type} required={required} defaultValue={defaultValue} placeholder={placeholder} disabled={disabled} className="h-12 rounded-xl border border-slate-200 bg-white px-4 font-semibold outline-none focus:border-[#0f6eb8] focus:ring-2 focus:ring-[#0f6eb8]/15 disabled:cursor-default disabled:bg-slate-50 disabled:text-slate-600" /></label>;
}

function SelectField({ label, name, options, defaultValue = "", required = true, disabled = false }: { label: string; name: string; options: Array<[string, string]>; defaultValue?: string; required?: boolean; disabled?: boolean }) {
  return <label className="grid gap-2 text-sm font-black text-[#081221]"><span>{label}{required ? " *" : ""}</span><select name={name} required={required} defaultValue={defaultValue} disabled={disabled} className="h-12 rounded-xl border border-slate-200 bg-white px-4 font-semibold outline-none focus:border-[#0f6eb8] focus:ring-2 focus:ring-[#0f6eb8]/15 disabled:cursor-default disabled:bg-slate-50 disabled:text-slate-600"><option value="">Select an option</option>{options.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>;
}
...
export function EnrollmentApplicationForm({ course, prefill, existing }: { course: Course; prefill: Prefill; existing: EnrollmentApplicationRecord | null }) {
  ...
  const readOnly = existing?.status === "submitted" || existing?.status === "approved";
  ...
  return <form action={submit} className="mx-auto max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(0,74,143,0.1)]">
    ...
    {existing ? <div className={`mx-6 mt-6 rounded-2xl border px-5 py-4 text-sm font-bold sm:mx-9 ${existing.status === "approved" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : existing.status === "changes_requested" ? "border-amber-200 bg-amber-50 text-amber-900" : "border-blue-200 bg-blue-50 text-blue-900"}`}>Status: {existing.status.replaceAll("_", " ")}{existing.reviewer_notes ? ` — ${existing.reviewer_notes}` : ""}{readOnly ? " — Your submitted answers are shown below in read-only mode." : ""}</div> : null}
    ...
    <div className="grid gap-8 p-6 sm:p-9">
      ...
      <button type="submit" disabled={submitting || readOnly} className="inline-flex h-14 items-center justify-center gap-2 rounded-full bg-[#0067b1] px-8 font-black text-white disabled:cursor-not-allowed disabled:opacity-60">{submitting ? <Loader2 className="animate-spin" size={19} /> : readOnly ? <CheckCircle2 size={19} /> : <Send size={19} />}{submitting ? "Submitting..." : existing?.status === "approved" ? "Application approved" : existing?.status === "submitted" ? "Application submitted" : "Submit application"}</button>
    </div>
    </fieldset>
  </form>;
}
