...
export function Cpp20218LlnTest({
  ...
  const [attempt, setAttempt] = useState<LlnAttemptSummary | null>(
    latestAttempt,
  );
  ...
  const groupedQuestions = useMemo(() => {
    return questions.reduce<Record<string, PublicLlnQuestion[]>>(
      (groups, question) => {
  ...
  const passedHeading = "Congratulations! You passed your Security LLN.";
  const passedCopy =
    "Your free LLN result is saved. Open your student portal using the email on your enrollment form. Cluster 1 stays locked until you pay AUD $150; SSTA manages later clusters.";
  ...
  async function submitTest() {
    ...
    try {
      ...
      setShowCongratulations(result.attempt.passed);
    } catch (submitError) {
      setError(
        submitError instanceof Error
          ? submitError.message
          : "Unable to submit LLN test.",
      );
    } finally {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      {showCongratulations && (
        <SecurityFlowDialog
          titleId="lln-congratulations"
          onClose={() => setShowCongratulations(false)}
        >
          <h2 id="lln-congratulations" className="text-2xl font-black">
            Congratulations! You passed your Security LLN.
          </h2>
          <p className="my-4 text-slate-600">
            Your result is saved. Sign in or create your student account with
            the same email as your enrollment form. Pay AUD $150 when you are
            ready to start Cluster 1.
          </p>
          <Link
            autoFocus
            href={returnTo}
            className="portal-button-primary inline-flex px-5 py-3"
          >
          ...
          <p className="mt-4 text-sm">
            <Link href="/enroll" className="text-sky-700 underline">
              Still need to submit your free enrollment form?
            </Link>
          </p>
        </SecurityFlowDialog>
      )}
      ...
      <section className="portal-card overflow-hidden rounded-[28px]">
        <div className="border-b border-[#d9e7f3] bg-[#f7fbff] p-7 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              <p className="portal-section-label">Free Security LLN</p>
              ...
              <p className="portal-page-copy mt-3 max-w-3xl">
                Complete this short MCQ check before purchasing or unlocking
                Certificate II Security Operations access.
              </p>
            </div>
            <div className="rounded-[18px] border border-[#d9e7f3] bg-white px-5 py-4 shadow-sm">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#6b7d90]">
                Pass mark
              </p>
        ...
        <div className="grid gap-8 p-6 sm:p-8">
          {Object.entries(groupedQuestions).map(
            ([section, sectionQuestions]) => (
              <div key={section} className="grid gap-4">
                <h2 className="text-xl font-black tracking-tight text-[#081221]">
                  {section}
                </h2>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-[20px] border border-[#d9e7f3] bg-[#f7fbff] p-4">
            ...
            <button
              type="button"
              onClick={submitTest}
              disabled={!canSubmit || !captchaToken || isSubmitting}
      ...
      <aside className="space-y-5">
        <section className="portal-card rounded-[28px] p-6">
          ...
          {attempt ? (
            ...
            <p className="mt-4 text-sm font-semibold leading-6 text-[#5d7389]">
              Answer all questions and submit when ready. Your result will be
              checked instantly.
            </p>
          )}
          ...
          {latestPassed ? (
            <div className="mt-5 rounded-[20px] border border-emerald-200 bg-emerald-50/70 p-5">
              <p className="text-base font-black leading-6 text-[#081221]">
                {passedHeading}
              </p>
              ...
              >
                Submit your free enrollment form
              </Link>
            </div>
          ) : attempt ? (
        ...
        <section className="portal-card rounded-[28px] p-6">
          ...
          <div className="mt-4 space-y-3 text-sm font-semibold leading-6 text-[#5d7389]">
            <p>
              Pass with 60% or higher, then sign in or create your student
              account using the email on your enrollment form.
            </p>
            <p>
              If you do not pass, you can retake the test. SSTA can also support
              you if any learning needs come up.
            </p>
            <p>
              Your course stays locked until you pay AUD $150 to start Cluster
              1. SSTA manages access to later clusters.
            </p>
          </div>
        </section>
      </aside>
