  ...
  return (
    <div className="mt-4">
      <p className="mb-3 text-sm font-semibold leading-6 text-[#5d7389]">
        Pay {amountLabel ?? "AUD $150"} to start Cluster 1. SSTA manages access to later clusters.
      </p>
      ...
      >
        <CreditCard size={16} />
        {isLoading ? "Starting payment..." : `Pay to start Cluster 1${amountLabel ? ` - ${amountLabel}` : ""}`}
      </button>
      {message ? (
