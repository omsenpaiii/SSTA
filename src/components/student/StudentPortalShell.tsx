...
export function StudentPortalShell({ user, stats, children }: StudentPortalShellProps) {
  ...
  return (
    <div className="min-h-screen bg-white text-[#081221]">
      <header className="sticky top-0 z-40 border-b border-[#dbe3ec] bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-[84px] max-w-[1480px] items-center justify-between gap-5 px-5 lg:px-8">
          <Link href="/dashboard" className="flex min-w-[210px] items-center gap-3">
            <span className="relative flex size-11 shrink-0 overflow-hidden rounded-[8px] bg-[#0f6eb8] p-1.5 shadow-sm">
              <Image
                src="/ssta.jpg"
                alt="SSTA logo"
                fill
            ...
            <span>
              <span className="block text-[1.55rem] font-black leading-none tracking-tight text-[#004b93]">
                SSTA
              </span>
              <span className="block text-xs font-bold uppercase tracking-[0.16em] text-[#5d7389]">
                Student Portal
              </span>
            </span>
          </Link>

          <nav className="hidden h-full min-w-0 flex-1 items-center justify-center gap-3 xl:flex">
            {navItems.map(({ href, label }) => {
              ...
              return (
                ...
                >
                  {label}
                  {active ? (
                    <span className="absolute inset-x-2 bottom-0 h-[3px] rounded-full bg-[#0f6eb8]" />
                  ) : null}
                </Link>
              );
            })}
          ...
          <div className="hidden items-center gap-3 sm:flex">
            <div className="flex items-center gap-3 rounded-[8px] border border-[#dbe3ec] bg-white px-3 py-2 shadow-sm">
              <span className="flex size-10 items-center justify-center rounded-full bg-[#0f6eb8] text-sm font-black text-white">
                {user.initials}
              </span>
              <span className="hidden text-left lg:block">
                <span className="block text-sm font-bold text-[#081221]">{user.name}</span>
                <span className="block text-xs font-medium text-[#64748b]">{user.email}</span>
              </span>
            </div>
            <form action="/auth/sign-out" method="post">
              <button className="inline-flex h-12 items-center gap-2 rounded-[8px] border border-[#dbe3ec] bg-white px-4 text-sm font-semibold text-[#081221] transition hover:border-[#0f6eb8] hover:text-[#0f6eb8]">
                <LogOut size={17} />
                Sign out
              </button>
            </form>
      ...
      <main className="mx-auto max-w-[1480px] px-5 py-6 lg:px-8">
        <section className="mb-5 grid gap-3 rounded-[8px] border border-[#dbe3ec] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] md:grid-cols-2 xl:grid-cols-4">
          {statItems.map((item) => {
            const Icon = item.icon;
            const value = stats[item.key];
            return (
              <article key={item.key} className="flex items-center gap-4">
                <div className={`flex size-11 shrink-0 items-center justify-center rounded-[8px] ${item.tone}`}>
                  <Icon size={21} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-[#475569]">{item.label}</p>
                  <p className="mt-1 text-2xl font-black leading-none text-[#081221]">{value}</p>
                </div>
                <Link
                  href={item.href}
                  className="hidden shrink-0 items-center gap-1 text-sm font-semibold text-[#0059c8] sm:inline-flex"
                >
                  {item.action}
                  <ArrowRight size={15} />
                </Link>
