...
export const CPP20218_LLN_COURSE_SLUG = "certificate-ii-security-operations";
export const CPP20218_LLN_TEST_KEY = "cpp20218-lln-v1";
export const CPP20218_LLN_PASS_PERCENT = 60;
...
const questions: LlnQuestion[] = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: "ask", label: "Ask your trainer or SSTA support for clarification" },
...
export function getPublicCpp20218LlnQuestions(): PublicLlnQuestion[] {
...
export function normalizeLlnReturnTo(value: string | null | undefined) {
  if (!value || !value.startsWith("/") || value.startsWith("//")) {
    return `/dashboard/course/${CPP20218_LLN_COURSE_SLUG}?tab=activities`;
  }
...
export function gradeCpp20218LlnAnswers(answers: Record<string, string>) {
  ...
  const passed = scorePercent >= CPP20218_LLN_PASS_PERCENT;
...
export async function getLatestCpp20218LlnAttempt(userKey: string) {
  ...
  const { data, error } = await supabase
    ...
    .eq("user_key", userKey)
    .eq("course_slug", CPP20218_LLN_COURSE_SLUG)
    .eq("test_key", CPP20218_LLN_TEST_KEY)
...
export async function hasPassedCpp20218Lln(userKey: string) {
  const supabase = getSupabaseAdmin();
...
export async function recordCpp20218LlnAttempt(input: {
  userKey: string;
  email: string;
  ...
  const result = gradeCpp20218LlnAnswers(input.answers);
  const { data, error } = await supabase
    .from("lln_attempts")
    .insert({
      user_key: input.userKey,
      email: input.email,
      course_slug: CPP20218_LLN_COURSE_SLUG,
      test_key: CPP20218_LLN_TEST_KEY,
      score: result.score,
      total: result.total,
